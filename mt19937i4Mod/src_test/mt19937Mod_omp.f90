...
contains
  ...
  function grnd_omp()
    ...
    double precision      :: grnd_omp

    if ( mti >= N ) then
       
       if ( mti == N+1 ) then
          call sgrnd_omp( seed_default )
       endif
    ...
    if ( y < 0 ) then
       grnd_omp = ( dble(y) + 2.0d0**32 ) / ( 2.0d0**32 )
    else
       grnd_omp =   dble(y)               / ( 2.0d0**32 )
    endif
    
  end function grnd_omp
  ...
  ! ====================================================== !
  subroutine multi_sgrnd_omp( seeds )
    !$ use omp_lib
    implicit none
    ...
    ! ------------------------------------------------------ !
    ! --- [1] iterate grnd_omp for each seed                --- !
    ! ------------------------------------------------------ !
    do k=1, OMPNumThreads
       multi_mt(0,k) = iand( seeds(k), maskF )
       do mti=1, N-1
          multi_mt(mti,k) = int(maskI) * ( ieor( multi_mt(mti-1,k), ( ishft( multi_mt(mti-1,k), -30) ) ) ) + mti
    ...
    return
  end subroutine multi_sgrnd_omp
  ...
  ! === random number generation                       === !
  ! ====================================================== !
  function multi_grnd_omp( myth )
    implicit none
    integer                :: kk
    integer, parameter     :: seed_default = 4357
    integer                :: seeds(OMPNumThreads)
    double precision       :: multi_grnd_omp
    integer                :: y
    ...
    if ( y < 0 ) then
       multi_grnd_omp = ( dble(y) + 2.0d0**32 ) / ( 2.0d0**32 )
    else
       multi_grnd_omp =   dble(y)               / ( 2.0d0**32 )
    endif
    
    return
  end function multi_grnd_omp

end module mt19937Mod_omp
