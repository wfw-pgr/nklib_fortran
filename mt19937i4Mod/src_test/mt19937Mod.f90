module mt19937Mod_mod
  implicit none
  integer, parameter :: OMPNumThreads = 8
...
contains
  ...
  ! === seed generation                                === !
  ! ====================================================== !
  subroutine sgrnd_mod( seed )
    implicit none
    integer, intent(in) :: seed

    mt(0) = iand( int(seed), maskF )
    do mti=1, N-1
  ...
  function grnd_mod()
    ...
    double precision      :: grnd_mod

    if ( mti >= N ) then
       
       if ( mti == N+1 ) then
          call sgrnd_mod( seed_default )
       endif
    ...
    if ( y < 0 ) then
       grnd_mod = ( dble(y) + 2.0d0**32 ) / ( 2.0d0**32 )
    else
       grnd_mod =   dble(y)               / ( 2.0d0**32 )
    endif
    
    return
  end function grnd_mod
  ...
  ! ====================================================== !
  subroutine multi_sgrnd_mod( seeds )
    !$ use omp_lib
    implicit none
    ...
    ! ------------------------------------------------------ !
    ! --- [1] iterate sgrnd_mod for each seed            --- !
    ! ------------------------------------------------------ !
    do k=1, OMPNumThreads
       multi_mt(0,k) = iand( int(seeds(k)), maskF )
       do mti=1, N-1
    ...
    return
  end subroutine multi_sgrnd_mod
  ...
  ! === random number generation                       === !
  ! ====================================================== !
  function multi_grnd_mod( myth )
    implicit none
    integer                :: kk
    integer, parameter     :: seed_default = 4357
    integer                :: seeds(OMPNumThreads)
    double precision       :: multi_grnd_mod
    integer             :: y
    integer, intent(in)    :: myth

    ! ------------------------------------------------------ !
    ! --- [1] iterate grnd_mod for every thread              --- !
    ! ------------------------------------------------------ !
    ...
    if ( y < 0 ) then
       multi_grnd_mod = ( dble(y) + 2.0d0**32 ) / ( 2.0d0**32 )
    else
       multi_grnd_mod =   dble(y)               / ( 2.0d0**32 )
    endif
    
    return
  end function multi_grnd_mod

end module mt19937Mod_mod
